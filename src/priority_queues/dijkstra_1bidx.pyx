...
cpdef cnp.ndarray path_length_from(
    ...
    min_heap_insert(&bheap, origin_vert + 1, 0.0)

    # main loop
    while bheap.size > 0:
        tail_vert_idx = extract_min(&bheap) - 1
        tail_vert_val = bheap.elements[tail_vert_idx + 1].key
        # loop on outgoing edges
        for edge_idx in range(csr_indptr[tail_vert_idx], csr_indptr[tail_vert_idx + 1]):
            head_vert_idx = csr_indices[edge_idx] + 1
